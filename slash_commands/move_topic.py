    ...
    for channel in interaction.guild.channels:
        if str(channel.mention) == channel_mention:
            ...
            if len(channel_has_topic) == 0:
                ...
                data_util.update_topic_information(topic_name=topic, guild_id=Interaction.guild_id, channel_id=channel_id)
                await interaction.followup.send(f"Moved {topic} from <#{channel_has_topic[0].chanel_id}> to {channel.mention}")
