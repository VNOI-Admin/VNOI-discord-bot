...
PLATFORM_LIST = ['Codeforces', 'Codechef', 'Atcoder']
PLATFORM_LIST.sort()
...
async def ask(interaction: discord.Interaction, platform: str, problem_id: str):
    ...
    if platform in PLATFORM_LIST:
        problem_name = await judge_api.fetch_problem_name(platform, problem_id)
        if not problem_name:
            await interaction.response.send_message("Invalid problem ID, please find more info about problem ID format with `/format` command")
